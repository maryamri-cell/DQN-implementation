...
import time
import matplotlib.colors as mcolors # Import ajouté pour des couleurs potentielles, mais pas utilisé directement ici

# Définir le device
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ==============================================================================
# 1. ENVIRONNEMENT GYMNASIUM : CustomGoalGridEnv (RENDER RÉINTÉGRÉ)
# ==============================================================================
...
class CustomGoalGridEnv(gym.Env):
    ...
    def close(self):
        if self.fig: # S'assurer que la figure existe avant de la fermer
            plt.close(self.fig)

# ==============================================================================
# 2. RÉSEAU NEURONAL PYTORCH (DQN) (INCHANGÉ)
# ==============================================================================
...
# ==============================================================================
# 3. AGENT DQN AVEC TARGET NETWORK (MODIFIÉ)
# ==============================================================================
...
class DQNAgentWithTarget:
    def __init__(self, state_size, action_size, learning_rate=1e-3, gamma=0.95, epsilon=1.0, update_target_freq=200):
        ...
        # Réseau Principal (Policy Network)
        self.model = DQN(state_size, action_size).to(DEVICE)
        
        # Réseau Cible (Target Network)
        self.target_model = DQN(state_size, action_size).to(DEVICE)
        self.target_model.load_state_dict(self.model.state_dict())
        self.target_model.eval() # Le réseau cible ne doit pas être entraîné
...
# ==============================================================================
# 4. BOUCLE PRINCIPALE D'EXÉCUTION (VISUALISATION RÉINTÉGRÉE)
# ==============================================================================
...
if __name__ == "__main__":
    ...
    # --- Hyperparamètres ---
    GRID_SIZE = 8
    N_EPISODES = 1000 # Nous allons exécuter plus d'épisodes pour mieux voir la convergence
    BATCH_SIZE = 64
    
    # --- NOUVEAU Hyperparamètre Target ---
    TARGET_UPDATE_FREQ = 200 # Mettre à jour le réseau cible tous les 200 pas de replay
    
    # --- Paramètres de Visualisation ---
    RENDER_INTERVAL = 100 # Affiche l'épisode de démo tous les 100 épisodes
    RENDER_DELAY = 0.05   # Délai en secondes entre chaque pas (peut être ajusté)
    ...
    # --- Utilisation du NOUVEL agent avec Target Network ---
    agent = DQNAgentWithTarget(state_size, action_size, update_target_freq=TARGET_UPDATE_FREQ)

    reward_history = []
    epsilon_history = []

    print(f"🚀 Démarrage de l'entraînement sur une grille {GRID_SIZE}x{GRID_SIZE} avec DQN/TARGET PyTorch ({DEVICE})...")
    ...
    try:
        for e in range(1, N_EPISODES + 1):
            ...
            is_demo_episode = (e % RENDER_INTERVAL == 0) # Déterminer si c'est un épisode de démo

            while not terminated and not truncated:
                
                # Visualisation dans la fenêtre Matplotlib (ACTIF SEULEMENT POUR LES ÉPISODES DE DÉMO)
                if is_demo_episode:
                    env.render() 
                ...
                # Ajout d'une condition de troncation pour éviter les épisodes infiniment longs
                if time_step > GRID_SIZE * GRID_SIZE * 5: # Max 5 fois le nombre de cellules
                    truncated = True
            
            # --- Entraînement et mise à jour des stats ---
            ...
            # Message spécial pour la fin de l'épisode de démo
            if is_demo_episode:
                ...
                time.sleep(1) # Laisser la dernière image visible un instant

    finally:
        env.close() # Ferme la fenêtre Matplotlib à la fin
